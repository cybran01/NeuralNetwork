...
public class NumberDetect extends JFrame {
	...
	public NumberDetect() {
		...
		try {
			...
			FileInputStream fin = new FileInputStream("Weights.dat");
		...
		drawpanel.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				...
				// draw the panel, subtracting the created scaled image (scaling the oldimg down
				// in the process)
				for (int j = 0; j < 28 * 28; j++) {
					int[] data = new int[1], olddata = drawpanel.img.getRaster().getPixel(factor * (j / 28),
							factor * (j % 28), (int[]) null);
		...
		btnDetect.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < 28 * 28; i++)
					...
					// drawpanel.img.getRaster().getPixel(factor*(i/28), factor*(i%28),
					// (int[])null)[0]));
							drawpanel.img.getRaster().getPixel(factor * (i / 28), factor * (i % 28),
									(int[]) null)[0])));
